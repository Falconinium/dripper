...
import { removeRoasterPhoto, uploadRoasterPhoto } from '../actions';

type Photo = { url: string; path: string; alt?: string };

export function RoasterPhotoManager({
  roasterId,
  photos,
}: {
  roasterId: string;
  ...
  const handleUpload = (file: File) => {
    ...
    startTransition(async () => {
      const res = await uploadRoasterPhoto(roasterId, fd);
        ...
        {photos.map((p) => (
          ...
          >
            ...
            <button
              ...
              onClick={() =>
                startTransition(async () => {
                  await removeRoasterPhoto(roasterId, p.path);
                })
              }
              disabled={isPending}
